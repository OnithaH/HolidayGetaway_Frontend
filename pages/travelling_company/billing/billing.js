...
async function fetchBills() {
    ...
    try {
        const res = await fetch('http://localhost:5000/api/travelCompany/billing', {
    ...
    bills.forEach(bill => {
        ...
        html += `
            <tr class="bill-row" data-status="${status.toLowerCase()}">
                ...
                <td>
                    <div class="room-info">
                        <span class="room-type">${bill.description || 'Blocked Booking Charge'}</span>
                    </div>
                </td>
                <td>-</td>
                <td>-</td>
                ...
                <td>
                    <div class="action-buttons">
                         ${status === 'Pending' ? `
                        <button class="btn btn-warning btn-sm" onclick="alert('Corporate payment processing...')">
                          <span>Pay Now</span>
                        </button>` : ''}
